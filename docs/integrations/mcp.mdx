...
- A Dograh API key. Generate one at `/api-keys` in your deployment, or `http://localhost:3010/api-keys` for local development. See [API Keys](/configurations/api-keys).
...
```bash
claude mcp add --transport http dograh https://your-backend.example.com/api/v1/mcp/ \
  --header "X-API-Key: YOUR_API_KEY"
```

Replace `YOUR_API_KEY` with the key you generated and use your deployment's backend URL.
...
```json
{
  "mcpServers": {
    "dograh": {
      "url": "https://your-backend.example.com/api/v1/mcp/",
      "headers": {
