...
2. Under **Capabilities** → **Voice**, confirm:
   - **Answer URL** is set to: `https://your-backend.example.com/api/v1/telephony/inbound/run`
   - **HTTP Method** is `POST`
...
<AccordionGroup>
  ...
  <Accordion title="Inbound calls not reaching voice agent">
    - Verify the Vonage application's Answer URL is set to `https://your-backend.example.com/api/v1/telephony/inbound/run`
