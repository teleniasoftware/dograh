...
Replace `your-dograh-instance` with your deployment's backend domain.
...
The id is shown on each row in **Telephony configurations** (`/telephony-configurations` in your deployment, or `http://localhost:3010/telephony-configurations` for local). The configuration must belong to the same organization as the API Trigger; otherwise the request returns `404`.
